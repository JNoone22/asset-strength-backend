...
def fetch_crypto_data(symbol, ma_period=20):
    """Fetch cryptocurrency data from CoinCap API (truly unlimited free tier)"""
    ...
    symbol_upper = symbol.upper().replace('-USD', '').replace('/USD', '')
    coin_id = coincap_map.get(symbol_upper, symbol.lower())
    
    try:
        # CoinCap history endpoint (daily data)
        days = ma_period * 7 + 30
        ...
        print(f"✓ CoinCap: Fetched {len(weekly_prices)} weeks for {symbol}")
        return weekly_prices
        
    except requests.exceptions.RequestException as e:
        print(f"✗ CoinCap error for {symbol}: {str(e)}")
        raise ValueError(f"CoinCap API request failed: {str(e)}")
    except (KeyError, ValueError) as e:
        print(f"✗ CoinCap parse error for {symbol}: {str(e)}")
...
def get_asset_data(symbol, ma_period=20):
    ...
    def fetch():
        if is_crypto(symbol):
            print(f"→ {symbol} detected as CRYPTO (using CoinCap)")
...
def calculate_relative_strength(asset1_data, asset2_data):
    ...
    INVERTED LOGIC: Returns strength of asset2 (column) relative to asset1 (row)
    - Positive strength (green) = column asset is stronger
    - Negative strength (red) = column asset is weaker
    """
    # Calculate ratios (asset2/asset1 instead of asset1/asset2)
    ratio = asset2_data['currentPrice'] / asset1_data['currentPrice']
    ma_ratio = asset2_data['ma'] / asset1_data['ma']
...
def health_check():
    ...
    return jsonify({
        ...
        'apis': {
            ...
            'coincap': 'Enabled (no key required)'
        }
    })
...
if __name__ == '__main__':
    print("=" * 70)
    print("Asset Strength Matrix - Backend API v3.1")
    print("Twelve Data (800 calls/day) + CoinCap (unlimited)")
    ...
    print(f"CoinCap: Enabled (no key required)")
