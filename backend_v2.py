"""
Asset Strength Matrix - Backend API Server v2.0
Daily refresh at 8am EST with Alpha Vantage + CoinGecko
"""
...
def get_next_8am_est():
    ...
    # If it's past 8am today, use tomorrow's 8am
    if now_est >= today_8am:
        next_8am = today_8am + timedelta(days=1)
    else:
        next_8am = today_8am
    
    # Return seconds until next 8am
...
def get_cached_or_fetch(cache_key, fetch_function):
    ...
    if cache_key in cache:
        ...
        if current_time < expires_at:
            print(f"Cache hit for {cache_key} (expires in {int((expires_at - current_time)/3600)}h)")
            return data
...
def fetch_alpha_vantage_data(symbol, ma_period=20):
    ...
    try:
        ...
        # Debug logging
        print(f"Alpha Vantage keys for {symbol}: {list(data.keys())}")
        ...
        # Check for the time series data with multiple possible keys
        time_series_key = None
        for key in data.keys():
            if 'Weekly Adjusted Time Series' in key or 'Time Series' in key:
                time_series_key = key
                break
        
        if not time_series_key:
            print(f"ERROR: No time series found for {symbol}. Response: {data}")
        ...
        closing_prices = [float(time_series[date]['5. adjusted close']) 
                         for date in sorted_dates]
        
        print(f"Successfully fetched {len(closing_prices)} weeks of data for {symbol}")
        
        return closing_prices
        
    except requests.exceptions.RequestException as e:
        print(f"Request error for {symbol}: {str(e)}")
        raise ValueError(f"Alpha Vantage API request failed: {str(e)}")
    except KeyError as e:
        print(f"Key error for {symbol}: {str(e)}")
        raise ValueError(f"Data format error for {symbol}")
...
def get_asset_data(symbol, ma_period=20):
    ...
    def fetch():
        if is_crypto(symbol):
            print(f"Detected {symbol} as cryptocurrency - using CoinGecko")
            closing_prices = fetch_crypto_data(symbol, ma_period)
            source = "CoinGecko"
        else:
            print(f"Detected {symbol} as stock/ETF - using Alpha Vantage")
            closing_prices = fetch_alpha_vantage_data(symbol, ma_period)
...
def health_check():
    ...
    return jsonify({
        ...
        'apis': {
            'alpha_vantage': 'Configured ✓' if ALPHA_VANTAGE_API_KEY != 'YOUR_API_KEY_HERE' else 'NOT CONFIGURED ✗',
...
if __name__ == '__main__':
    print("=" * 60)
    print("Asset Strength Matrix - Backend API v2.0")
    print("Daily Refresh at 8:00 AM EST")
    print("=" * 60)
    print(f"Alpha Vantage: {'Configured ✓' if ALPHA_VANTAGE_API_KEY != 'YOUR_API_KEY_HERE' else 'NOT CONFIGURED ✗'}")
